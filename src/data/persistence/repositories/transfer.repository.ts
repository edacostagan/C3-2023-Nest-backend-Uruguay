...
export class TransferRepository extends BankInternalControl<TransferEntity> implements RepositoryMethodsInterface<TransferEntity> {
    ...
    /**
     * Return all the historical data (IN/OUT) of an account
     * @param accountId account to be found
     * @param pagination elements to return
     * @param dataRange range of dates to look for
    ...
    getAllTransfersById(accountId: string,
        ...
        try { 
            ...
            let searchResult = this.database.filter(transfer => (transfer.outcome == accountId
                                                    || transfer.income == accountId)
